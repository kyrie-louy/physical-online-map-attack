...
def load_config():
    ...
    config_path = Path(__file__).parent.parent.parent.parent / "configs" / "HybridAStar.yaml"
...
# Replace global constants with config values
XY_GRID_RESOLUTION = CONFIG['grid']['xy_resolution']
YAW_GRID_RESOLUTION = CONFIG['grid']['yaw_resolution']
MOTION_RESOLUTION = CONFIG['grid']['motion_resolution']
N_STEER = CONFIG['grid']['n_steer']

SB_COST = CONFIG['costs']['switch_back']
BACK_COST = CONFIG['costs']['backward']
STEER_CHANGE_COST = CONFIG['costs']['steering_change']
STEER_COST = CONFIG['costs']['steering']
H_COST = CONFIG['costs']['heuristic']

GOAL_RADIUS = CONFIG['goal']['radius']
GOAL_HEADING_RANGE = CONFIG['goal']['heading_range']
...
def hybrid_a_star_planning(start, goal, ox, oy, xy_resolution, yaw_resolution, 
                           grid_params=None, cost_params=None, goal_params=None):
    """
    start: start node
    goal: goal node
    ...
    grid_params: Optional dict of grid parameters
    cost_params: Optional dict of cost parameters
    """
    # Use provided parameters or fall back to defaults from config file
    if grid_params:
        global XY_GRID_RESOLUTION, YAW_GRID_RESOLUTION, MOTION_RESOLUTION, N_STEER
        XY_GRID_RESOLUTION = grid_params['xy_resolution']
        YAW_GRID_RESOLUTION = grid_params['yaw_resolution']
        MOTION_RESOLUTION = grid_params['motion_resolution']
        N_STEER = grid_params['n_steer']
        
    if cost_params:
        global SB_COST, BACK_COST, STEER_CHANGE_COST, STEER_COST, H_COST
        SB_COST = cost_params['switch_back']
        BACK_COST = cost_params['backward'] 
        STEER_CHANGE_COST = cost_params['steering_change']
        STEER_COST = cost_params['steering']
        H_COST = cost_params['heuristic']
        
    if goal_params:
        global GOAL_RADIUS, GOAL_HEADING_RANGE
        GOAL_RADIUS = goal_params['radius']
        GOAL_HEADING_RANGE = goal_params['heading_range']
    ...
    while True:
        ...
        # Check if current node is within goal region
        dist_to_goal = np.hypot(current.x_list[-1] - goal[0],
                               current.y_list[-1] - goal[1])
        heading_diff = abs(rs.pi_2_pi(current.yaw_list[-1] - goal[2]))
        
        if dist_to_goal <= GOAL_RADIUS and heading_diff <= GOAL_HEADING_RANGE:
            # print("reach goal region!")
            # Create terminal node with proper parent linkage
            final_path = current
            break
