...
class LiDARInstanceLines(object):
    ...
    """
    def __init__(self, 
                 instance_line_list,  # a list of shapely LineString objects
                 sample_dist=1,
                 num_samples=250,
                 padding=False,
                 fixed_num=-1,
    ...
    @property
    def fixed_num_sampled_points_v2(self):
        """
        return torch.Tensor([N,fixed_num,2]), in xmin, ymin, xmax, ymax form
            N means the num of instances
        """
    ...
    @property
    def fixed_interval_sampled_points(self):
        """
        Return torch.Tensor([N, variable_num, 2]), where points are sampled at fixed distance intervals
        N is the number of instances, and variable_num depends on the length of each instance
        """
        assert len(self.instance_list) != 0
        instance_points_list = []
        for instance in self.instance_list:
            # Calculate the number of points based on the instance length and desired interval
            num_points = max(2, int(instance.length / self.sample_dist) + 1)
            distances = np.linspace(0, instance.length, num_points)
            sampled_points = np.array([list(instance.interpolate(distance).coords) for distance in distances]).reshape(-1, 2)
            instance_points_list.append(sampled_points)
        
        # Pad or truncate to ensure all instances have the same number of points
        max_points = max(len(points) for points in instance_points_list)
        padded_points_list = []
        for points in instance_points_list:
            if len(points) < max_points:
                padding = np.full((max_points - len(points), 2), self.padding_value)
                padded_points = np.vstack((points, padding))
            else:
                padded_points = points[:max_points]
            padded_points_list.append(padded_points)
        
        instance_points_array = np.array(padded_points_list)
        instance_points_tensor = to_tensor(instance_points_array)
        instance_points_tensor = instance_points_tensor.to(dtype=torch.float32)
        mask_x = instance_points_tensor[:,:,0] != self.padding_value
        mask_y = instance_points_tensor[:,:,1] != self.padding_value
        instance_points_tensor[:,:,0] = torch.where(mask_x, torch.clamp(instance_points_tensor[:,:,0], min=-self.max_x, max=self.max_x), torch.tensor(self.padding_value, dtype=torch.float32))
        instance_points_tensor[:,:,1] = torch.where(mask_y, torch.clamp(instance_points_tensor[:,:,1], min=-self.max_y, max=self.max_y), torch.tensor(self.padding_value, dtype=torch.float32))
        return instance_points_tensor
...
class VectorizedLocalMap(object):
    ...
    def gen_vectorized_samples(self, location, lidar2global_translation, lidar2global_rotation):
        ...
        vectors = []  # a list of tuples, each tuple contains a shapely geometry object and a label
        ...
        # convert shapely geometry objects to instance points
    ...
    def get_map_geom(self, patch_box, patch_angle, layer_names, location):
        ...
        for layer_name in layer_names:
            if layer_name in self.line_classes:
                geoms = self.get_divider_line(patch_box, patch_angle, layer_name, location)  # a list of shapely LineString objects
    ...
    def _one_type_line_geom_to_instances(self, line_geom):
        ...
        for line in line_geom:
            if not line.is_empty:
                ...
                elif line.geom_type == 'LineString':  # enter here
    ...
    def poly_geoms_to_instances(self, polygon_geom):
        ''' polygon exterior and interior to LineString objects '''
        ...
        results = []
        for ext in exteriors:  # polygon.LinearRing
            if ext.is_ccw:
                ext.coords = list(ext.coords)[::-1]
            # notes: intersection breaks the closed ring
            lines = ext.intersection(local_patch)  # MultiLineString
            if isinstance(lines, MultiLineString):
                lines = ops.linemerge(lines)
            results.append(lines)
    ...
    def get_contour_line(self,patch_box,patch_angle,layer_name,location):
        ...
        if layer_name == 'drivable_area':
            ...
            for record in records:
                ...
                if polygon.is_valid:  # check if polygon is well-formed (have a closed ring)
                    new_polygon = polygon.intersection(patch)
                    if not new_polygon.is_empty:
                        new_polygon = affinity.rotate(new_polygon, -patch_angle,
                                                      origin=(patch_x, patch_y), use_radians=False)
        ...
        return polygon_list  # a list of shapely MultiPolygon objects at this patch
    ...
    def get_divider_line(self,patch_box,patch_angle,layer_name,location):
        ...
        patch = self.map_explorer[location].get_patch_coord(patch_box, patch_angle)  # a polygon

        line_list = []
        records = getattr(self.map_explorer[location].map_api, layer_name)  # all records of this layer
        for record in records:
        ...
        return line_list  # a list of shapely LineString objects at this patch
...
class CustomNuScenesLocalMapDataset(CustomNuScenesDataset):
    ...
    def get_data_info(self, index):
        ...
        if self.modality['use_camera']:
            ...
            for cam_type, cam_info in info['cams'].items():
                ...
                global2ego = np.eye(4).astype(np.float32)
                # Inverse rotation: transpose of the rotation matrix
                global2ego[:3, :3] = Quaternion(input_dict['ego2global_rotation']).rotation_matrix.T
                # Inverse translation: -R^T * t
                global2ego[:3, 3] = -global2ego[:3, :3] @ input_dict['ego2global_translation']
                # ego to camera transform (inverse of camera2ego)
                ego2camera = np.linalg.inv(camera2ego)
                # combine transforms: first transform from global to ego, then ego to camera
                global2camera = ego2camera @ global2ego
                # combine with camera intrinsics to get global to image pixels transform
                global2img = viewpad @ global2camera
                input_dict["global2img"].append(global2img)
    ...
    def to_gt_vectors(self,
                      gt_dict):
        # import pdb;pdb.set_trace()
        # check if gt_labels_3d are data container
        if isinstance(gt_dict['gt_labels_3d'], DC):
            gt_labels = gt_dict['gt_labels_3d'].data[0][0]
        else:
            gt_labels = gt_dict['gt_labels_3d']
        if isinstance(gt_dict['gt_bboxes_3d'], DC):
            gt_instances = gt_dict['gt_bboxes_3d'].data[0][0].instance_list
        else:
            gt_instances = gt_dict['gt_bboxes_3d'].instance_list
        ...
        for gt_instance, gt_label in zip(gt_instances, gt_labels):
            ...
            gt_vectors.append({
                'pts': pts,  # (x, y), x in [-30, 30], y in [-15, 15]
                'pts_num': pts_num,
                'type': int(gt_label)
            })
        vector_num_list = {}
    ...
    def _evaluate_single(self,
        ...
        with open(result_path,'r') as f:  # /home/yanglou3/workspace/MapTR/test/maptr_tiny_r50_24e_bevformer/Wed_Nov_20_21_14_48_2024/pts_bbox/nuscmap_results.json
            pred_results = json.load(f)
        gen_results = pred_results['results']
        with open(self.map_ann_file,'r') as ann_f:  # data/nuscenes/nuscenes_map_anns_val.json
    ...
    def evaluate(self,
        ...
        if save_results:
            ...
            # Skip saving formatted results but still evaluate
            # Create a temporary directory for evaluation
            tmp_dir = tempfile.TemporaryDirectory()
            # Format results but save to temporary directory
            temp_jsonfile_prefix = os.path.join(tmp_dir.name, 'temp_results')
            result_files, _ = self.format_results(results, temp_jsonfile_prefix)
